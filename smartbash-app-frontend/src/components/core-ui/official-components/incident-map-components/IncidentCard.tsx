...
export function IncidentCard({ type, urgency, reports, location }: IncidentCardProps) {
  ...
  return (
    <div className={`border-l-4 ${borderColor} bg-white rounded-lg p-4 shadow-sm`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-semibold text-gray-500 uppercase">Urgency Level</span>
        <span className="px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full capitalize">{urgency}</span>
      </div>
