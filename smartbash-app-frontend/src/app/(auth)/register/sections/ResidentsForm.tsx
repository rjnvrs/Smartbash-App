...
interface ResidentFormProps {
  formData: ResidentsFormData;
  updateField: (field: keyof ResidentsFormData, value: string) => void;
  onFileChange: (file: File) => void;
}

export default function ResidentsForm({ formData, updateField, onFileChange }: ResidentFormProps) {
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <InputField
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => updateField("middleName", e.target.value)}
        />
        <InputField
          label="Last Name"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <InputField
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => updateField("age", e.target.value)}
          />
          <InputField
            label="Gender"
            placeholder="Gender"
            defaultValue={formData.gender}
            onChange={(e: ChangeEvent<HTMLInputElement>) => updateField("gender", e.target.value)}
          />
        </div>
      </div>
