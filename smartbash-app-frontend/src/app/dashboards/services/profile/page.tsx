...
export default function ServiceProfilePage() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        setProfileImage(profile.avatarUrl || defaultProfile);
      } catch (error) {
    ...
    reader.onloadend = () => {
      const upload = async () => {
        try {
          ...
          const res = await apiFetch("/auth/services/profile/avatar/", {
            method: "POST",
            headers: {},
            body: form,
          });
          ...
          setProfileImage(data?.avatarUrl || defaultProfile);
  ...
  const handleSave = () => {
    ...
    const save = async () => {
      try {
        const res = await apiFetch("/auth/services/profile/update/", {
          method: "POST",
          body: JSON.stringify({
  ...
  const handlePasswordSave = () => {
    ...
    const savePassword = async () => {
      try {
        const res = await apiFetch("/auth/services/profile/password/", {
          method: "POST",
          body: JSON.stringify({
