...
import {
  ServiceStatus,
  ServiceCategory,
} from "@/components/core-ui/official-components/services-components/ActionBar";
...
export default function Services() {
  ...
  const loadServices = async () => {
    ...
    setError("");
    try {
      const res = await apiFetch("/auth/officials/services/", {
        method: "GET",
      });
      const { data, text } = await parseJsonSafe(res);
      if (!res.ok) {
        if (!data) throw new Error(text || "Failed to load services");
  ...
  const handleAddService = async (newService: Omit<Service, "id">) => {
    try {
      ...
      const { data, text } = await parseJsonSafe(res);
      if (!res.ok) {
        if (!data) throw new Error(text || "Create failed");
  ...
  const handleDeleteService = async (id: number) => {
    try {
      ...
      const { data, text } = await parseJsonSafe(res);
      if (!res.ok) {
        if (!data) throw new Error(text || "Delete failed");
